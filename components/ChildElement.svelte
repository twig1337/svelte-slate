<script context="module">import { createContextKey, getFromContext } from '../utils';
const SELECTED_CONTEXT_KEY = createContextKey();
export function getSelectedContext() {
    return getFromContext(SELECTED_CONTEXT_KEY);
}
</script><script>import { Range, Editor } from 'slate';
...
$: childDecorations = getChildDecorations(decorate([currentElement, childPath]), range, currentDecorations);
