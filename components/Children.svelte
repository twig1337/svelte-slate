<script context="module">import { Range } from 'slate';
export function getChildDecorations(childDecorations, range, decorations = []) {
...
</script><script>import { Element as SlateElement, Editor } from 'slate';
...
</script>{#each currentNode.children as child, index (findKey(child))}{#if SlateElement.isElement(child)}<ChildElement
			{decorations}
			{selection}
			element={child}
			parent={currentNode}
