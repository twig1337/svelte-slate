<script context="module">import { Editor, Node as SlateNode, Element as SlateElement, Path, Range, Transforms } from 'slate';
...
import { defaultDecorate, getDecorateContext, getEditorContext, getFocusedContext, getReadOnlyContext, getSelectionContext } from './Slate.svelte';
import { HAS_BEFORE_INPUT_SUPPORT, IS_CHROME, IS_FIREFOX, IS_FIREFOX_LEGACY, IS_IOS, IS_QQBROWSER, IS_SAFARI } from '../environment';
import hotkeys from '../hotkeys';
import { DOMNode, DOMRange, getDefaultView, isDOMElement, isPlainTextOnlyPaste } from '../dom';
import { isDOMNode } from '../dom';
import { createContext, createContextKey, findDocumentOrShadowRoot, findEventRange, findPath, focus, getFromContext, hasDOMNode, hasRange, isFocused, toDOMNode, toDOMRange, toSlateNode, toSlateRange } from '../utils';
import { EDITOR_TO_ELEMENT, EDITOR_TO_WINDOW, ELEMENT_TO_NODE, IS_FOCUSED, IS_READ_ONLY, NODE_TO_ELEMENT, PLACEHOLDER_SYMBOL } from '../weakMaps';
import { getContainerContext } from './Slate.svelte';
import { isHotkey } from '../isHotkey';
export const ELEMENT_CONTEXT_KEY = createContextKey();
export const LEAF_CONTEXT_KEY = createContextKey();
export const PLACEHOLDER_CONTEXT_KEY = createContextKey();
...
export function hasTarget(editor, target) {
    return isDOMNode(target) && hasDOMNode(editor, target);
}
export function hasEditableTarget(editor, target) {
    return isDOMNode(target) && hasDOMNode(editor, target, { editable: true });
}
export function isTargetInsideNonReadonlyVoid(editor, target) {
...
export function defaultScrollSelectionIntoView(_editor, domRange) {
...
</script><script>export let Element = DefaultElement;
export let Leaf = DefaultLeaf;
export let Placeholder = DefaultPlaceholder;
...
let decorations = [];
$: if ($editorContext.children.length === 1 &&
...
function onDOMSelectionChange() {
    if (state.isComposing || state.isDraggingInternally || state.updateSelectionTimeoutId) {
        ...
        const anchorNodeSelectable = hasEditableTarget(editor, domSelection.anchorNode) ||
            isTargetInsideNonReadonlyVoid(editor, domSelection.anchorNode);
        const focusNodeSelectable = hasEditableTarget(editor, domSelection.focusNode) ||
...
function onUpdate() {
    ...
    const newDomRange = editor.selection && hasDomSelectionInEditor && toDOMRange(editor, editor.selection);
    if (newDomRange) {
...
function onDragStart(event) {
    if (!state.readOnly && hasTarget(editor, event.target)) {
        ...
        editor.setFragmentData(event.dataTransfer);
...
><Children node={$editorContext} selection={$selectionContext} {decorations} /></div><style>
	div {
		position: relative;
		outline: none;
